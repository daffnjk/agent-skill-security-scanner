...
- [v0.2.0 泛化评测快照](v41/README.md)：8 个同时包含正负标签的冻结数据集，补充误报率、准确率、平衡准确率及扫描完整性统计；机器可读指标见 [机器可读指标](v41/metrics.csv)。
- [v0.1.0 初始评测快照](v38/README.md)：13 个本地语料评估单元及独立 PoisonedSkills 正样本集；完整机器可读指标见 [机器可读指标](v38/metrics.csv)。
...
发布新检测器版本时，应保留已有 CSV 字段；可以在尾部追加诊断字段。应尽可能保持数据集版本和材料化方式不变，并在 manifest 与 README 中明确记录任何变化。
